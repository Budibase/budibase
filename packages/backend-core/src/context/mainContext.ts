...
export function getWorkspaceId(): string | undefined {
  const context = Context.get()
  const foundId = context?.workspaceId
...
export async function ensureSnippetContext() {
  ...
  if (db) {
    const workspace = await db.tryGet<App>(DocumentType.APP_METADATA)
...
export function getDevWorkspaceDB(opts?: any): Database {
  ...
  return getDB(conversions.getDevelopmentWorkspaceID(workspaceId), opts)
}
