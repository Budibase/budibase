...
describe("app", () => {
  ...
  describe("getAppFileUrl env toggle (MinIO)", () => {
    const testKey = "app_123/attachments/image.jpeg"
    ...
    it("returns full direct MinIO URL when expiry disabled", async () => {
      env._set("ATTACHMENT_URL_EXPIRY_DISABLED", "1")

      const url = await app.getAppFileUrl(testKey)
      expect(url).toBe(
        `http://minio.example.com/prod-budi-app-assets/${testKey}`
      )
    })

    it("returns signed URL when expiry not disabled", async () => {
      const url = await app.getAppFileUrl(testKey)
      expect(url).toBe(`/files/signed/prod-budi-app-assets/${testKey}`)
    })
  })
})
