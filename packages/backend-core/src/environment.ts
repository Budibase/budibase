...
function getEnvironment() {
  return {
    ...
    DISABLE_CONTENT_SECURITY_POLICY:
      process.env.DISABLE_CONTENT_SECURITY_POLICY,
    BSON_BUFFER_SIZE: parseIntSafe(process.env.BSON_BUFFER_SIZE),
    init,
  }
}

// needs a constant reference for default export
const environment = getEnvironment()

export function init() {
  const newEnv = getEnvironment()
  for (let key of Object.keys(newEnv)) {
    environment._set(key, newEnv[key as keyof typeof newEnv])
  }
}
