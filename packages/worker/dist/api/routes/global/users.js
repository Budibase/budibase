...
function buildInviteMultipleValidation() {
    // prettier-ignore
    return joiValidator.body(Joi.array().required().items(Joi.object({
        email: Joi.string(),
        userInfo: Joi.object().optional(),
    })));
}
...
router
    .post("/api/global/users", adminOnly, users.buildUserSaveValidation(), controller.save)
    .post("/api/global/users/bulkCreate", adminOnly, users.buildUserBulkSaveValidation(), controller.bulkCreate)
    ...
    .post("/api/global/users/invite", adminOnly, buildInviteValidation(), controller.invite)
    .post("/api/global/users/invite", adminOnly, buildInviteValidation(), controller.invite)
    .post("/api/global/users/multi/invite", adminOnly, buildInviteMultipleValidation(), controller.inviteMultiple)
    // non-global endpoints
    .post("/api/global/users/invite/accept", buildInviteAcceptValidation(), controller.inviteAccept)
    .post("/api/global/users/init", cloudRestricted, buildAdminInitValidation(), controller.adminUser)
    ...
    .post("/api/global/users/self", users.buildUserSaveValidation(true), selfController.updateSelf);
