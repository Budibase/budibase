...
var app = (function () {
    	...
    	const block = {
    		...
    		p: function update(changed, ctx) {
    			if (changed.loginButtonLabel) {
    				set_data_dev(t, ctx.loginButtonLabel);
    			}
    		},
    ...
    function create_fragment$3(ctx) {
    	var div7, div6, t0, div4, div0, t1, t2, div1, updating_value, t3, div2, t4, t5, div3, updating_value_1, t6, div5, t7, current;

    	var if_block0 = (ctx.logo) && create_if_block_1$1(ctx);
    	...
    	if (ctx.username !== void 0) {
    		textbox0_props.value = ctx.username;
    	}
    	...
    	let textbox1_props = { hideValue: "true" };
    	if (ctx.password !== void 0) {
    		textbox1_props.value = ctx.password;
    	}
    	...
    	var button = new Button({
    		props: {
    		disabled: ctx.busy,
    		class: ctx.buttonClass,
    		$$slots: { default: [create_default_slot] },
    		$$scope: { ctx }
    	},
    		$$inline: true
    	});
    	button.$on("click", ctx.login);
    	...
    	const block = {
    		...
    		p: function update(changed, ctx) {
    			...
    			if (!current || changed.usernameLabel) {
    				set_data_dev(t1, ctx.usernameLabel);
    			}

    			var textbox0_changes = {};
    			if (!updating_value && changed.username) {
    				textbox0_changes.value = ctx.username;
    			}
    			textbox0.$set(textbox0_changes);

    			if (!current || changed.passwordLabel) {
    				set_data_dev(t4, ctx.passwordLabel);
    			}

    			var textbox1_changes = {};
    			if (!updating_value_1 && changed.password) {
    				textbox1_changes.value = ctx.password;
    			}
    			textbox1.$set(textbox1_changes);

    			var button_changes = {};
    			if (changed.busy) button_changes.disabled = ctx.busy;
    			if (changed.buttonClass) button_changes.class = ctx.buttonClass;
    			if (changed.$$scope || changed.loginButtonLabel) button_changes.$$scope = { changed, ctx };
