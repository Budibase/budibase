...
function createChannel(streamIn) {
  ...
  let handleIncomingPacket = (bytes) => {
    if (isFirstPacket) {
      ...
      if (binaryVersion !== "0.12.18") {
        throw new Error(`Cannot start service: Host version "${"0.12.18"}" does not match binary version ${JSON.stringify(binaryVersion)}`);
      }
      return;
    }
...
var _a;
var isInternalWorkerThread = ((_a = worker_threads == null ? void 0 : worker_threads.workerData) == null ? void 0 : _a.esbuildVersion) === "0.12.18";
...
var ensureServiceIsRunning = () => {
  ...
  let child = child_process.spawn(command, args.concat(`--service=${"0.12.18"}`, "--ping"), {
    windowsHide: true,
...
var runServiceSync = (callback) => {
  ...
  callback(service);
  let stdout = child_process.execFileSync(command, args.concat(`--service=${"0.12.18"}`), {
    cwd: defaultWD,
    windowsHide: true,
    input: stdin,
...
var startWorkerThreadService = (worker_threads2) => {
  ...
  let worker = new worker_threads2.Worker(__filename, {
    workerData: { workerPort, defaultWD, esbuildVersion: "0.12.18" },
