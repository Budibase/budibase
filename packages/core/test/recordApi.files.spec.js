...
describe("recordApi > files", () => {
  it("upload should fail when files size does not equal stream size", async () => {
    ...
    expect(
      recordApi.uploadFile(record.key, stream, file.relativePath)
    ).rejects.toThrow()
  })
  ...
  it("upload should fail when record does not exist", async () => {
    ...
    await recordApi.save(record)
    expect(
      recordApi.uploadFile("does nto exist", stream, file.relativePath)
    ).rejects.toThrow()
  })
  ...
  it("download should get an uploaded file", async () => {
    ...
    const downloadedStream = await recordApi.downloadFile(
      record.key,
      file.relativePath
    )
  ...
  it("upload should fail when path contains '..' ", async () => {
    ...
    await recordApi.save(record)
    expect(
      recordApi.uploadFile(record.key, stream, "../somefile.txt")
    ).rejects.toThrow()
  })
})
