...
export type FinalizeRunSummaryInput =
  | FunctionRunResult
  | {
      status: "error"
...
export const finalizeRunSummary = async (
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (!summary) {
      throw new Error("Function run summary not found")
    }
  ...
  throw new Error("Function run summary could not be finalized")
}
...
const compareRunsNewestFirst = (
  a: Pick<FunctionRunSummary, "startedAt" | "runId" | "environment">,
  b: Pick<FunctionRunSummary, "startedAt" | "runId" | "environment">
) =>
