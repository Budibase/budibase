...
For number inputs, normalize explicit, unambiguous numeric language to a numeric value when needed, such as "hundred" to "100". Treat a singular article directly describing one requested countable item as "1", such as "a laptop" or "an adapter". Do not apply this rule to vague quantities such as "a few" or "several". The source quote must contain the user's exact numeric language, but does not need to contain the normalized value verbatim.
