...
import { ContextUser, User, Database } from "@budibase/types"

export function combineMetadataAndUser(
  user: ContextUser,
  metadata: User[]
): (ContextUser & { _deleted?: boolean }) | null {
...
export async function rawUserMetadata(db?: Database): Promise<User[]> {
  if (!db) {
    db = context.getAppDB()
  }
  return (
...
export async function fetchMetadata(): Promise<ContextUser[]> {
