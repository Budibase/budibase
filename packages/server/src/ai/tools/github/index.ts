    ...
    return [
      ...
      newTool({
        ...
        parameters: z.object({
          owner: z.string().describe("The GitHub username or organization that owns the repository"),
          repo: z.string().describe("The repository name"),
          state: z.enum(["open", "closed", "all"]).optional().describe("State of issues to return")
        }),
      ...
      newTool({
        ...
        parameters: z.object({
          ...
          body: z.string().optional().describe("New content/description of the issue"),
        ...
        handler: async ({ owner, repo, issue_number, title, body, state, labels }) => {
          try {
      ...
      newTool({
        ...
        parameters: z.object({
          owner: z.string().describe("The GitHub username or organization that owns the repository"),
          repo: z.string().describe("The repository name"),
          state: z.enum(["open", "closed", "all"]).optional().describe("State of pull requests to return")
        }),
        handler: async ({ owner, repo, state = "open" }) => {
          try {
      ...
      newTool({
        ...
        parameters: z.object({
          ...
          ref: z.string().optional().describe("The git reference (branch or tag) to run the workflow on")
        }),
        handler: async ({ owner, repo, workflow_id, ref = "main" }) => {
          try {
            ...
            return JSON.stringify({ success: true, message: "Workflow dispatch created" }, null, 2)
          } catch (error: any) {
            return `Error: ${error.message}`
          }
        }
