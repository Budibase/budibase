...
export function generateColumnDefinition(config: {
  ...
  } else if (lowerCaseType === "array" && userDefinedType &&
             (userDefinedType.toLowerCase().includes("json") || userDefinedType.toLowerCase().includes("jsonb"))) {
    // Handle PostgreSQL JSON array types specifically - treat as JSON rather than array
    matchingTypes.push({ external: userDefinedType.toLowerCase(), internal: FieldType.JSON })
