...
describe("rowProcessor utility", () => {
    ...
    describe("fixAutoColumnSubType", () => {
        ...
        it("updates the schema with the correct subtype", async () => {
            ...
            expect(fixAutoColumnSubType(schema).subtype).toEqual(AutoFieldSubTypes.CREATED_BY)
            schema.subtype = ''

            schema.name = AutoFieldDefaultNames.UPDATED_BY
            expect(fixAutoColumnSubType(schema).subtype).toEqual(AutoFieldSubTypes.UPDATED_BY)
            schema.subtype = ''

            schema.name = AutoFieldDefaultNames.CREATED_AT
            expect(fixAutoColumnSubType(schema).subtype).toEqual(AutoFieldSubTypes.CREATED_AT)
            schema.subtype = ''

            schema.name = AutoFieldDefaultNames.UPDATED_AT
            expect(fixAutoColumnSubType(schema).subtype).toEqual(AutoFieldSubTypes.UPDATED_AT)
            schema.subtype = ''

            schema.name = AutoFieldDefaultNames.AUTO_ID
            expect(fixAutoColumnSubType(schema).subtype).toEqual(AutoFieldSubTypes.AUTO_ID)
            schema.subtype = ''
