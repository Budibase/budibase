...
describe("/screens", () => {
  ...
  describe("used plugins management", () => {
    ...
    it("should prune unused plugin on publish when plugin component is removed from screen", async () => {
      const screen = await config.api.screen.save(
      ...
      await config.doInContext(config.prodWorkspaceId!, async () => {
        const prodApp = await sdk.workspaces.metadata.get()
        expect(prodApp.usedPlugins?.length).toEqual(1)
      })

      screen.props._children = []
      await config.api.screen.save(screen)

      // On screen save, fast addition behavior preserves dev usedPlugins until publish
      await config.publish()

      await config.doInContext(config.prodWorkspaceId!, async () => {
        const prodApp = await sdk.workspaces.metadata.get()
        expect(prodApp.usedPlugins?.length).toEqual(0)
      })
    })

    it("should prune unused plugin on publish when screen containing it is deleted", async () => {
      const screen = await config.api.screen.save(
        createPluginScreen("/plugin-screen")
      )
    ...
    it("should retain plugin on publish if at least one screen is still using it", async () => {
