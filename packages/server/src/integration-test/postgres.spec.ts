  ...
  describe.each(mainDescriptions)(
    ...
    ({ config, dsProvider }) => {
      ...
      describe("check custom column types", () => {
        ...
        it("should handle JSON array columns as JSON type", async () => {
          await client.schema.createTable("jsonArrayTable", table => {
            table.increments("id").primary()
            table.specificType("json_array", "json[]")
            table.specificType("jsonb_array", "jsonb[]")
          })
          ...
          expect(response.datasource.entities).toBeDefined()
          const table = response.datasource.entities?.["jsonArrayTable"]
          expect(table).toBeDefined()
          expect(table?.schema.json_array.type).toBe(FieldType.JSON)
          expect(table?.schema.json_array.externalType).toBe("ARRAY")
          expect(table?.schema.jsonb_array.type).toBe(FieldType.JSON)
          expect(table?.schema.jsonb_array.externalType).toBe("ARRAY")
        })

        it("should handle JSON array data with nested objects", async () => {
          await client.schema.createTable("jsonArrayDataTable", table => {
            table.increments("id").primary()
            table.string("name")
            table.specificType("json_array", "json[]")
            table.specificType("jsonb_array", "jsonb[]")
          ...
          await client("jsonArrayDataTable").insert([
            {
              name: "Company A",
              json_array: [
              ...
              jsonb_array: [
                '{"budget": 50000, "department": "IT"}',
                '{"budget": 30000, "department": "HR"}'
              ]
            },
            {
              name: "Company B",
              ...
              jsonb_array: [
                '{"budget": 40000, "department": "Marketing"}'
              ]
            }
          ])
          ...
          const table = response.datasource.entities?.["jsonArrayDataTable"]
          expect(table).toBeDefined()

          // Query the data to verify it can be retrieved
          const { rows } = await config.api.row.search(table!._id!, {})
          expect(rows).toHaveLength(2)

          // Verify structure of returned data
          expect(rows[0].name).toBe("Company A")
          expect(rows[0].json_array).toHaveProperty("values")
          expect(Array.isArray(rows[0].json_array.values)).toBe(true)
          expect(rows[0].json_array.values[0]).toMatchObject({name: "John", role: "manager"})
          expect(rows[0].json_array.values[1]).toMatchObject({name: "Alice", role: "developer"})

          expect(rows[0].jsonb_array).toHaveProperty("values")
          expect(Array.isArray(rows[0].jsonb_array.values)).toBe(true)
          expect(rows[0].jsonb_array.values[0]).toMatchObject({budget: 50000, department: "IT"})

          // Test that properties are accessible (for Handlebars compatibility)
          expect(rows[0].json_array.values[0]).toHaveProperty("name")
          expect(rows[0].json_array.values[0]).toHaveProperty("role")
          expect(Object.hasOwnProperty.call(rows[0].json_array.values[0], "name")).toBe(true)
          expect(Object.hasOwnProperty.call(rows[0].json_array.values[0], "role")).toBe(true)

          // Test that "values" is an own property
          expect(Object.hasOwnProperty.call(rows[0].json_array, "values")).toBe(true)
        })

        it("should create virtual tables for JSON array fields", async () => {
          await client.schema.createTable("jsonArrayVirtualTable", table => {
            table.increments("id").primary()
            table.string("name")
            table.specificType("json_array", "json[]")
          })
          ...
          // Insert test data
          await client("jsonArrayVirtualTable").insert([
            {
              name: "Test Company",
              json_array: [
                '{"name": "John", "role": "manager"}',
          ...
          console.log("Available tables:", Object.keys(entities || {}))

          // Check that virtual table was created
          const virtualTableName = "jsonArrayVirtualTable.json_array.values"
          const virtualTable = entities?.[virtualTableName]
          expect(virtualTable).toBeDefined()
          expect(virtualTable?.schema.parent_id).toBeDefined()
          expect(virtualTable?.schema.name).toBeDefined()
          expect(virtualTable?.schema.role).toBeDefined()
        })
      })
