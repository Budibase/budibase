...
export const isRootComponent = c => isComponent(c) && isUndefined(c.props._component)
...
export const searchAllComponents = (components, phrase) => {
  const hasPhrase = (...vals) => pipe(vals, [some(v => includes(normalString(phrase))(normalString(v)))])
...
export const getAncestorProps = (components, name, found = []) => {
  ...
  return getAncestorProps(components, thisComponent.props._component, [{ ...thisComponent.props }, ...found])
}
