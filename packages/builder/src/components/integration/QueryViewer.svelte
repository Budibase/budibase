<script>
  ...
  const resetSchemaFields = () => {
    fields = query?.schema ? schemaToFields(query.schema) : []
  }
  ...
  $: queryId, initData()
  $: saveId, resetSchemaFields()

  // fields and data need cleared if the query is changed.
