<script>
  ...
  function setAutomationLive(live) {
    ...
    if (live) {
      notifications.info(
        $t("automation") + ` ${automation.name} ${$t("enabled")}.`
      )
    } else {
      notifications.error(
        `${$t("automation")} ${automation.name} ${$t("disabled")}.`
      )
    }
  }
  ...
  async function testAutomation() {
    ...
    if (result.status === 200) {
      notifications.success(
        `${$t("automation")} ${automation.name} ${$t(
          "triggered-successfully"
        )}.`
      )
    } else {
      notifications.error(
        `${$t("failed-to-trigger-automation")} ${automation.name}.`
      )
    }
  }
  ...
  async function saveAutomation() {
    ...
    notifications.success(
      `${$t("automation")} ${automation.name} ${$t("saved")}.`
    )
  }
</script>
