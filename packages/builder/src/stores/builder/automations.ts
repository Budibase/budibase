...
export const createAutomationStore = () => {
  const store = writable<AutomationState>(initialAutomationState)
  store.update((state: any) => {
    return {
      ...state,
      actions: automationActions(store),
    }
  })

  const history = createHistoryStore({
    getDoc: () => get(store).actions.getDefinition(),
    selectDoc: (id: string) => get(store).actions.select(id),
    beforeAction: () => {},
    afterAction: () => {},
  })
  ...
  store.update((state: any) => {
    return {
      ...
      actions: {
        ...state.actions,
        save: history.wrapSaveDoc(state.actions.save),
        delete: history.wrapDeleteDoc(state.actions.delete),
      },
    }
  })
...
const getFinalDefinitions = (triggers: any, actions: any) => {
  const creatable: { [key: string]: any } = {}
...
const automationActions = (store: any) => ({
  /**
   ...
   */
  deleteBlock: (pathTo: any, automation: Automation) => {
    ...
    pathTo.forEach((path, pathIdx, array) => {
      ...
      const deleteCore = (steps: any, idx: number) => {
   ...
   */
  getPathSteps: (pathWay: any, automation: Automation) => {
  ...
  updateStep: (
    ...
    const finalise = (dest: any, idx: number, update: any) => {
      dest.splice(
        idx,
  ...
  buildEnvironmentBindings: () => {
    if (get(licensing).environmentVariablesEnabled) {
      return getEnvironmentBindings().map((binding: any) => {
        return {
          ...binding,
          display: {
            ...binding.display,
  ...
  buildSettingBindings: () => {
    return getSettingBindings().map((binding: any) => {
      return {
        ...binding,
        display: {
          ...binding.display,
   ...
   */
  traverse: (blockRefs: any, automation: Automation) => {
    let blocks: any[] = []
   ...
   * @returns
   */
  getAvailableBindings: (block: any, automation: Automation) => {
  ...
  processBlockInputs: async (block: any, data: any) => {
  ...
  updateBlockInputs: async (block: any, data: any) => {
  ...
  getUpdatedDefinition: (automation: Automation, block: any) => {
    let newAutomation
  ...
  addTestDataToAutomation: async (data: any) => {
  ...
  constructBlock(
    type: string,
    stepId: string,
    blockDefinition: AutomationStepDefinition
  ) {
    let newName
    const newStep = {
      ...blockDefinition,
   ...
   */
  addBlockToAutomation: async (block: any, pathWay: any) => {
    const automation = get(selectedAutomation).data
    let newAutomation = cloneDeep(automation)
    ...
    pathWay.forEach((path, pathIdx, array) => {
      ...
      const insertBlock = (steps: any, stepIdx: number) => {
   ...
   */
  branchAutomation: async (path: any, automation: Automation) => {
   ...
   * @param {Object} block
   */
  branchLeft: async (pathTo: any, automation: Automation, block: any) => {
   ...
   * @param {Object} block
   */
  branchRight: async (pathTo: any, automation: Automation, block: any) => {
   ...
   * @returns
   */
  shiftBranch(pathTo: any, block: any, direction = -1) {
   ...
   */
  deleteBranch: async (path: any, automation: Automation) => {
    let newAutomation = cloneDeep(automation)
    let cache = []
   ...
   */
  deleteAutomationBlock: async (pathTo: any) => {
    const automation = get(selectedAutomation)?.data
  ...
  replace: async (automationId: string, automation: Automation) => {
    if (!automation) {
      store.update(state => {
        // Remove the automation
        state.automations = state.automations.filter(
...
export const selectedAutomation = derived(automationStore, $automationStore => {
  if (!$automationStore.selectedAutomationId === null) {
    return null
  }
