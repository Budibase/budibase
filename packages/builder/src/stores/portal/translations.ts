...
class TranslationsStore extends BudiStore<TranslationsState> {
  ...
  private getActiveLocale(state: TranslationsState, locale?: string) {
    const config = normaliseConfig(state.config)
  ...
  async init(force = false) {
    ...
    const configDoc =
      ...
      "type" in response &&
      isTranslationsConfig(response as Config)
        ? (response as TranslationsConfig)
        : undefined
    const config = normaliseConfig(configDoc?.config)
    this.set({ loaded: true, config })
  }
