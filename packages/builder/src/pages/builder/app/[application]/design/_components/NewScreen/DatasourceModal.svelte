<script>
  import { ModalContent, Layout, notifications } from "@budibase/bbui"
  ...
  import tableSelectFormat from "helpers/data/tableSelectFormat"
  import datasourceSelectFormat from "helpers/data/datasourceSelectFormat"
  ...
  const getViews = table => {
    ...
    return views.map(view => ({
      ...
      name: view.name,
      id: view.id,
      tableSelectFormat: tableSelectFormat.viewV2(view),
      datasourceSelectFormat: datasourceSelectFormat.viewV2(view),
    }))
  }
  ...
  const getTablesAndViews = datasource => {
    ...
    for (const table of tables) {
      ...
      const formattedTable = {
        ...
        name: table.name,
        id: table._id,
        tableSelectFormat: tableSelectFormat.table(table),
        datasourceSelectFormat: datasourceSelectFormat.table(
          table,
...
<ModalContent
  title="Choose a table to create your screen"
  confirmText="Next"
  cancelText="Cancel"
  {onConfirm}
