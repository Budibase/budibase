<script>
  ...
  $: {
    if ($datasources.list.length && !datasource) {
      $redirect(`/builder/workspace/${$params.workspaceId}/apis`)
    } else if (datasource && !isRestDatasource) {
      $redirect(
        `/builder/workspace/${$params.workspaceId}/data/query/new/${$params.datasourceId}`
      )
    } else if (datasource && isRestDatasource) {
      queries.setNewQueryDatasourceId($params.datasourceId)
      $redirect(`/builder/workspace/${$params.workspaceId}/apis/query/new`)
    }
  }
</script>
