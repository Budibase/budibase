...
const getPastClientVersion = async () => {
  const manifestRaw = await fetch(
    `https://api.github.com/repos/budibase/budibase/contents/packages/client/manifest.json?ref=v${version}`
  ).then(r => r.json())
  ...
  await fs.mkdir(`packages/server/build/oldClientVersions/${version}`, {
    recursive: true,
  })
  await fs.writeFile(
    `packages/server/build/oldClientVersions/${version}/manifest.json`,
    manifest
  )

  const npmRegistry = await fetch(
    `https://registry.npmjs.org/@budibase/client/${version}`
  ).then(r => r.json())

  // The json response from npm is just a string starting with the following if the version can't be found
  if (
    typeof npmRegistry === "string" &&
    npmRegistry.startsWith("version not found")
  ) {
  ...
  await exec(
    `curl -L ${npmRegistry.dist.tarball} --output - | tar -xvzf - -C clientVersionTmp`
  )

  // Copy the compiled client version we want to the oldClientVersions dir and delete the temp directory
  await fs.copyFile(
    "./clientVersionTmp/package/dist/budibase-client.js",
    `./packages/server/build/oldClientVersions/${version}/app.js`
  )
  ...
  const rootDir = await fs.readdir("packages/server/build/oldClientVersions/", {
    withFileTypes: true,
  })
