...
describe("license management", () => {
  ...
  it("retrieves plans, creates checkout session, and updates license", async () => {
    ...
    // Retrieve plans
    const [, planBody] = await config.api.licenses.getPlans()
    ...
    await config.doInNewState(async () => {
      ...
      const [, selfBodyPremium] = await config.api.accounts.self()
      expect(selfBodyPremium.license.plan.type).toBe(PlanType.PREMIUM_PLUS)

      // Create portal session - Check URL
      const [, portalSessionBody] = await config.api.stripe.createPortalSession(
        customer.id
      )
      ...
      const [, selfBodyBusiness] = await config.api.accounts.self()
